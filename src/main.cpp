...
void Prostokat(int szer,int wys,int x,int y);
void kropki(float);
void wskaznik(void);
...
void Timer2Isr()
{
	if(++preScale==500)
	{
		...
		Model.Calculate();
		//Wiatrak.setInput(syg_ster);
		//Wiatrak.Calculate();

		//powietrze = Wiatrak.getOutput();

		//Pomieszczenie.setInput(syg_ster, zaklo);
		//Pomieszczenie.Calculate();
